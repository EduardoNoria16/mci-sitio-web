...
let out = "const CATEGORY_IMAGES: Record<string, string[]> = {\n";
...
out += "const ALL_IMAGES = CATEGORIES.flatMap(cat => {\n" +
...
const oldFile = fs.readFileSync('src/components/ProjectGallery.tsx', 'utf-8');

const updated = oldFile.replace(
  /const FALLBACK_IMAGES = \[[\s\S]*?\];[\s\S]*?const ALL_IMAGES = CATEGORIES\.flatMap[\s\S]*?\}\);/m,
  out
);

fs.writeFileSync('src/components/ProjectGallery.tsx', updated);
